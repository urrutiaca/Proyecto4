...
# ---
# 
# * Estudiante: **Ana Carolina Urrutia Núñez**
# * Carné: **B77833**
# * Grupo: **2**
# 
# ---
...
def modulador(bits, fc, mpp):
    ...
    moduladora_Q = np.zeros(t_simulacion.shape)  # (opcional) señal de bits
    ...
    ### Se obtiene la señal:
    senal_Tx = senal_Q + senal_I
    
    
    # 5. Calcular la potencia promedio de la señal modulada
    P_senal_Tx = (1 / (N*Tc)) * np.trapz(pow(senal_Tx, 2), t_simulacion)
    
    return senal_Tx, P_senal_Tx, portadora_1,portadora_2, moduladora_I, moduladora_Q 
...
def demodulador(senal_Rx, portadora_1, portadora_2, mpp):
    ...
    # Vector para la señal demodulada PARA 16 QAM
    senal_demodulada_I = np.zeros(senal_Rx.shape)
    senal_demodulada_Q = np.zeros(senal_Rx.shape)

    # Pseudo-energía de un período de la portadora
    Es = np.sum(portadora_1 * portadora_2)
    ...
    for i in range(N):
        ...
        producto_I = senal_Rx[i*mpp : (i+1)*mpp] * portadora_1
        producto_Q = senal_Rx[i*mpp : (i+1)*mpp] * portadora_2
        Ep_I = np.sum(producto_I)
        Ep_Q = np.sum(producto_Q)
        Ep_1 = np.abs(producto_I)
        Ep_2 = np.abs(producto_Q)
...
# ### 3.2. - Simulación del sistema de comunicaciones con modulación 16-QAM
# 
...
senal_Tx, P_senal_Tx, portadora_1,portadora_2, moduladora_I, moduladora_Q = modulador(bits_Tx, fc, mpp)
...
bits_Rx,senal_demodulada_I,senal_demodulada_Q = demodulador(senal_Rx,portadora_1,portadora_2,mpp)
...
ax3.plot(senal_demodulada_I[0:600], color='b', lw=2) 
ax3.set_ylabel('$Demodulada_I$')

# La señal demodulada_Q
ax4.plot(senal_demodulada_Q[0:600], color='m', lw=2) 
ax4.set_ylabel('$Demodulada_Q$')
...
### Ergodicos si los promedios temporales son iguales a los estadisticos
## Ya se tiene la señal Tx, se busca promedio de una señal periodica:

P1 = np.mean(senal_Tx[0:20])
P2 = np.mean(senal_Tx)
print("Ergocidad:")
print("Comparación de modelos, donde el primer valor corresponde a periodico y el segundo al estático:",P1,P2)
print("Los valores difieren por lo que se puede decir que no hay ergocidad")


## Estacionaridad: si las propiedades estadisticas no cambian en el tiempo

P3 = np.var(senal_Tx)
print("\nEstacionaridad:\nSe obtiene la varianza",P3,"Y como vemos, su número es cercano a 1, lo que indica que no existen grandes diferencias al rededor de la media")
